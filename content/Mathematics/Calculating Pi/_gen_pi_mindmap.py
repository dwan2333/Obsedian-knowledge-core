"""Generate pi_mindmap.png — concept map for the Calculating Pi note.

Central node: how pi was calculated, with 5 surrounding nodes for the 5 major topics.
"""
import matplotlib.pyplot as plt
import matplotlib.patches as mpatches
from math import cos, sin, pi

fig, ax = plt.subplots(figsize=(13.5, 10))

# Center node
center_w, center_h = 4.6, 1.9
center = mpatches.FancyBboxPatch(
    (-center_w / 2, -center_h / 2), center_w, center_h,
    boxstyle='round,pad=0.1,rounding_size=0.3',
    facecolor='#1f4f8c', edgecolor='#0f2f5c', linewidth=2.0, zorder=4
)
ax.add_patch(center)
ax.text(0, 0.35, r'Calculating $\pi$',
        fontsize=16, ha='center', va='center',
        color='white', fontweight='bold', zorder=5)
ax.text(0, -0.30, 'From polygons (centuries) to series (minutes)',
        fontsize=11, ha='center', va='center',
        color='white', zorder=5, style='italic')

topics = [
    ("1. Why $\\pi$?",
     "Circle area + circumference" "\n"
     r"$A = \pi r^2$, $C = 2\pi r$" "\n"
     "Pizza-slice proof"),
    ("2. Polygon method",
     r"Hexagon: $\pi > 3$" "\n"
     r"Square: $\pi < 4$" "\n"
     "Bisect: 12, 24, 48, 96..." "\n"
     "van Ceulen $2^{62}$ sides $\\to$ 35 digits"),
    ("3. Binomial expansion",
     r"$(1+x)^n$ for any $n$" "\n"
     "Coefficients = Pascal's triangle" "\n"
     "Newton extends to fractional $n$" "\n"
     "$\\to$ infinite series"),
    ("4. Pascal extended",
     "Above: $n = -1, -2, ...$" "\n"
     "Between: $n = 1/2, 1/3, ...$" "\n"
     "Same add-neighbors rule" "\n"
     "$\\to$ $\\sqrt{1-x^2}$ as a series"),
    ("5. Calculus shortcut",
     r"$\int_0^{1/2}\sqrt{1-x^2}\,dx$" "\n"
     r"$= \frac{\pi}{12} + \frac{\sqrt{3}}{8}$" "\n"
     "Solve for $\\pi$" "\n"
     "5 terms $\\to$ 3.14161"),
]

node_palette = [
    ('#e2924a', '#8c4f1f'),  # orange
    ('#4a90e2', '#1f4f8c'),  # blue
    ('#7bb55c', '#3d7530'),  # green
    ('#b76db4', '#6e3a6c'),  # purple
    ('#d4a04a', '#8c6520'),  # yellow
]

n_topics = len(topics)
radius = 6.0
box_w, box_h = 4.0, 2.3

for i, ((title, body), (fc, ec)) in enumerate(zip(topics, node_palette)):
    theta = pi / 2 - i * (2 * pi / n_topics)
    x = radius * cos(theta)
    y = radius * sin(theta)
    ax.plot([0, x], [0, y], color='#bbbbbb', linewidth=1.5, zorder=1)
    box = mpatches.FancyBboxPatch(
        (x - box_w / 2, y - box_h / 2), box_w, box_h,
        boxstyle='round,pad=0.08,rounding_size=0.25',
        facecolor=fc, edgecolor=ec, linewidth=2.0, alpha=0.94, zorder=2
    )
    ax.add_patch(box)
    ax.text(x, y + box_h / 2 - 0.32, title,
            fontsize=12, ha='center', va='center',
            color='white', fontweight='bold', zorder=3)
    ax.text(x, y - 0.25, body,
            fontsize=10, ha='center', va='center',
            color='white', zorder=3)

ax.set_title("Calculating Pi — concept map (Veritasium video)",
             fontsize=15, pad=10, fontweight='bold')
ax.set_xlim(-9, 9); ax.set_ylim(-8.5, 8.5)
ax.set_aspect('equal'); ax.axis('off')

plt.tight_layout()
plt.savefig('pi_mindmap.png', dpi=220,
            bbox_inches='tight', facecolor='white')
plt.close()
print("Saved pi_mindmap.png")
