...
>
> > [!success]- Click to reveal solution
> > **(a)** A's hand is an unordered 13-card subset of the deck:
> > $$\binom{52}{13} \;=\; 635{,}013{,}559{,}600$$
> >
> > **(b)** Deal sequentially — A picks 13 of 52, B picks 13 of the remaining 39, C picks 13 of 26, D gets the last 13:
> > $$\binom{52}{13}\binom{39}{13}\binom{26}{13}\binom{13}{13} \;=\; \frac{52!}{(13!)^4} \;\approx\; 5.36 \times 10^{28}$$
> >
> > This is the **multinomial coefficient**.
> >
> > > [!warning] Do NOT divide by $4!$
> > > A natural instinct says "the four hands are interchangeable, divide by $4!$ for overcounting." Wrong here. The problem says *"it matters which player gets which hand"* — the hands are **labeled** by player. "A holds the spades" is genuinely different from "B holds the spades." No symmetry to remove.
> > >
> > > You'd divide by $4!$ only if the problem said *"split the deck into four anonymous 13-card piles"* — that's [Example 1.5.4 generalized](<Chapter 1 (Main).md>) to 4 groups of 13: $\dfrac{52!}{(13!)^4 \cdot 4!}$.
> >
> > **(c)** $\binom{52}{13}^4$ would mean "give each player 13 cards from a *fresh* deck" — sampling **with replacement**, where the same Ace of Spades could land in every hand at once. Real dealing is **without replacement**: A's pick shrinks B's pool to 39, B's shrinks C's to 26, etc. The hands are *not independent*; they share one finite deck.
> >
> > That single constraint shrinks the count by a factor of about $3 \times 10^{18}$:
> >
> > | | Value |
> > |---|---|
> > | $\binom{52}{13}^4$ (with replacement) | $\approx 1.6 \times 10^{47}$ |
> > | $\dfrac{52!}{(13!)^4}$ (without replacement) | $\approx 5.4 \times 10^{28}$ |
> >
> > **Answer.** (a) $\dbinom{52}{13}$  — (b) $\dfrac{52!}{(13!)^4}$  — (c) the four hands share one deck ✓
> >
> > > [!tip] One-line takeaway
> > > **Labeled** recipients → multinomial. **Anonymous** piles → multinomial ÷ $g!$. **Fresh** draws → power. **Shared** deck → multinomial.
